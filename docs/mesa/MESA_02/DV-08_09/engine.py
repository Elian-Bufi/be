...
# ── C4 ──
def c4(x,y,w,h,name,tech,desc,kind="cont"):
    col={"person":C["green"],"cont":C["blue"],"sys":C["blue"],"ext":C["amber"],"db":C["blue"]}[kind]
    fill={"person":"#FFFFFF","cont":"#FFFFFF","sys":C["blueSoft"],"ext":C["amberSoft"],"db":"#FFFFFF"}[kind]
    dash=' stroke-dasharray="5 3"' if kind=="ext" else ''
    o=[]
    if kind=="db":
        o.append(f'<path d="M{x-w/2:.0f} {y-h/2+10:.0f}a{w/2:.0f} 10 0 0 1 {w:.0f} 0v{h-20:.0f}a{w/2:.0f} 10 0 0 1 -{w:.0f} 0z" fill="{fill}" stroke="{col}" stroke-width="1.9"/>')
        o.append(f'<path d="M{x-w/2:.0f} {y-h/2+10:.0f}a{w/2:.0f} 10 0 0 0 {w:.0f} 0" fill="none" stroke="{col}" stroke-width="1.3"/>')
    else:
        o.append(f'<rect x="{x-w/2:.0f}" y="{y-h/2:.0f}" width="{w}" height="{h}" rx="9" fill="{fill}" stroke="{col}" stroke-width="{2.1 if kind=="sys" else 1.8}"{dash}/>')
    yy=y-h/2+(30 if kind=="db" else 24)
    o.append(T(x,yy,name,12,C["ink"],700))
    if tech: o.append(T(x,yy+16,f"[{tech}]",10,col,600))
    if desc:
        for i,l in enumerate(desc.split("|")): o.append(T(x,yy+(34 if tech else 18)+i*13,l,10,C["grey"],500))
    return "".join(o),(x,y,w/2,h/2)
def boundary(x0,y0,x1,y1,label):
    return (f'<rect x="{x0}" y="{y0}" width="{x1-x0}" height="{y1-y0}" rx="12" fill="none" stroke="{C["blue"]}" stroke-width="1.3" stroke-dasharray="8 5" opacity=".55"/>'
            + T(x0+16,y0+20,label,11.5,C["blue"],700,"start"))
def rel4(a,b,txt,tech="",curve=0):
    x1,y1,w1,h1=a; x2,y2,w2,h2=b
    if abs(x2-x1)>abs(y2-y1): sx,sy,ex,ey=x1+(w1 if x2>x1 else -w1),y1,x2-(w2+9 if x2>x1 else -(w2+9)),y2
    else: sx,sy,ex,ey=x1,y1+(h1 if y2>y1 else -h1),x2,y2-(h2+9 if y2>y1 else -(h2+9))
    mx,my=(sx+ex)/2,(sy+ey)/2+curve
    d=f'M{sx:.0f} {sy:.0f}Q{mx:.0f} {my:.0f} {ex:.0f} {ey:.0f}' if curve else f'M{sx:.0f} {sy:.0f}L{ex:.0f} {ey:.0f}'
    o=[f'<path d="{d}" stroke="{C["blue"]}" stroke-width="1.5" fill="none" marker-end="url(#ab)" opacity=".85"/>']
    ls=[txt]+([f'[{tech}]'] if tech else [])
    bw=max(len(l) for l in ls)*5.5+14
    ly=my+(curve*0.5 if curve else 0)
    o.append(f'<rect x="{mx-bw/2:.0f}" y="{ly-9-(len(ls)-1)*6:.0f}" width="{bw:.0f}" height="{14+(len(ls)-1)*12}" rx="4" fill="#FFFFFF" opacity=".95"/>')
    for i,l in enumerate(ls): o.append(T(mx,ly+3-(len(ls)-1)*6+i*12,l,9.5,C["grey"] if i else C["ink"],600))
    return "".join(o)
