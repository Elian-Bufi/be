...
> **Estado:** **CERRADO** el 2026-09-24. Definición en `main` antes del código (PR #73); tramos A, B, C y E en el PR #74; D y F, con la versión 0.9.0, en el PR de cierre. Defensa en `DEFENSA/IDENTIDAD.md`, evidencia en `EVIDENCIA/IDENTIDAD/`. El tramo E se integró con A, B y C —no aparte, como decía §9— porque la prueba de contraste cubre las dos superficies y no podía quedar a medias. **Ajuste posterior del tramo D** (PR #76): al capturar en `test` apareció el cambio de protocolo con mediciones ya cargadas; el reparto entre campos y «fuera del protocolo» quedó en una sola regla que aplica B10-07 §17 —no agrega ninguna—, verificado en `EVIDENCIA/IDENTIDAD/verificacion-figura.md`.
