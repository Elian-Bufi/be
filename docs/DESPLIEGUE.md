...
- **Excepción WP-08 (desde 0.10.0):** volver a una revisión anterior a WP-08 **después de que exista al menos una importación** no tiene recuperación validada. La migración es aditiva, pero el dato lleva el valor de enum `CONTROLLED_IMPORT` en `procedencia`, que el código viejo no sabe presentar; PostgreSQL no permite quitar un valor de enum, y remapearlo a `PROFESSIONAL_MANUAL` conserva las filas pero **falsea la procedencia visible** (RF-060), así que no es una recuperación correcta. Ante un problema, la respuesta preferida es una **corrección hacia adelante sobre 0.10.0** que conserve su contrato y los datos originales, no el rollback. Hallazgo y detalle en `EVIDENCIA/WP-08/recuperacion-controlled-import.md`.
