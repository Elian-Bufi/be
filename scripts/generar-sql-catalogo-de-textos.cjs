 ...
 * Uso: npm run build:domain && node scripts/generar-sql-catalogo-de-textos.cjs [--ids id1,id2]
 * Con --ids emite solo esas versiones: cada migración nueva siembra solo sus textos, porque los anteriores ya están
 * en la base.
 */
...
const ids = arg > 0 ? new Set(process.argv[arg + 1].split(',')) : null;
const lineas = ['-- Catálogo de textos versionados (08 §12.2; DEUDA_LEGAJO DL-028). Generado desde @be/domain.'];
for (const v of CATALOGO_DE_TEXTOS.filter((t) => !ids || ids.has(t.id))) {
  ...
  const columnas = ['"id"', '"tipo"', '"titulo"', '"finalidad"', '"texto"', '"hash"', '"vigente_desde"'];
  const valores = [
    `'${v.id}'`,
    `'${v.tipo}'`,
    `${ETIQUETA}${v.titulo}${ETIQUETA}`,
    `'${v.finalidad}'`,
    `${ETIQUETA}${v.texto}${ETIQUETA}`,
    `'${v.hash}'`,
    `'${v.vigenteDesde}'`,
  ];
  if (v.reemplazaA) {
    columnas.push('"reemplaza_a_id"');
    valores.push(`'${v.reemplazaA}'`);
  }
  lineas.push(`INSERT INTO "version_de_texto" (${columnas.join(', ')}) VALUES (${valores.join(', ')});`);
}
