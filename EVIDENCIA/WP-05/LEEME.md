...
| `web/web-01` a `web-19` (.png) | El website en `test` (API/web 0.5.1, commit `02c252f`), con Edge real por puppeteer: el estado final de cada paso — borrador, registro, cálculo, referencia, corrección, anulación, evolución y el borrador ajeno que no existe. DEMO-PA, una asesorada sintética nueva y DEMO-PN |
...
- **`web-14` y `web-15`** — corregir conserva el valor original y dispara un recálculo que reemplaza al cálculo anterior sin borrarlo (REG-06-161), visible en el mismo cálculo con «Reemplaza a un cálculo anterior, que se conserva.».
