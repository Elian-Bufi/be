...
/** Lo mismo que exige la API (API-TRN-21): hasta 92 días, y el comienzo no después del final. */
...
export function FiltroDePeriodo({ id, onAplicar }: { id: string; onAplicar: (p: Periodo) => void }) {
  ...
  function aplicar(e: FormEvent) {
    ...
    if (desde && dias(desde, fin) >= DIAS_MAXIMOS) return setError({ campo: 'desde', texto: `El período puede abarcar hasta ${DIAS_MAXIMOS} días.` });
    setError(null);
