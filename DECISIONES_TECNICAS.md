...
| URLs de Render | `be-api-hndp.onrender.com`, `be-web-1ngj.onrender.com` | Los subdominios `onrender.com` son globales y `be-api`/`be-web` ya eran de terceros. La URL de la API queda en el rewrite de `render.yaml` y en `API_BASE_URL` de `eas.json`; la del website, solo en `CORS_ALLOWED_ORIGINS`. El 2026-09-18 el Blueprint se recreó sobre el repositorio nuevo: adoptó `be-api` y `be-db-test` (URL intacta) y creó un `be-web` nuevo (`izpg` → `1ngj`). Detalle en `docs/DESPLIEGUE.md`. |
