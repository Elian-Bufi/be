...
| Website | Render Static Site con export estático de Next. Desde WP-02 llama a la API **directo, con CORS** (`BE_API_BASE_URL` en el build); ya no hay rewrite `/api/*` | 07 CAND-07-J opción C (sin segundo runtime Node) → DL-007. La parte same-origin por rewrite se abandonó por decisión de Dirección (DL-030): detrás del rewrite la API no veía la IP real, que la evidencia de A1/A2 necesita (08 §12.2). Cabeceras HSTS, CSP, X-Frame-Options y nosniff en `render.yaml`. La CSP incluye `'unsafe-inline'` para scripts porque el export estático de Next usa scripts inline; se endurece con hashes cuando haya funcionalidad. |
...
**Regresión y decisión final (2026-09-18).** En el repositorio recreado, el merge del PR #1 (`960d1d2`) se hizo con `gh pr merge`. GitHub firma los merges web con el email de la cuenta, así que ese commit volvió a tener el email personal del autor, y el PR lo referencia de forma permanente. Se evaluaron tres salidas: repositorio espejo privado para Render, repositorio y Blueprint nuevos, o publicar tal cual. **Dirección decidió publicar tal cual**: el email del autor queda visible solo en ese merge commit. Antes de publicar se repitió la auditoría sobre los 16 commits y los 323 blobs del repositorio: sin credenciales, sin el PDF y sin datos de terceros. Medida para que no se repita: todos los merges se hacen en local con el email noreply (`eaf785c`, `fd3ed53`); se recomienda activar «Keep my email addresses private» en la cuenta.
